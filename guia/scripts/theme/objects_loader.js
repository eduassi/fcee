...
var refresh_objects_listeners = function () {
  ...
  // QUICK QUIZ BOX LOADER

  function buildTimeline(count, active) {
    count_question = count + 1;

    if (count_question > 1) {
      active_question = active + 1;
      let balls_on_the_timeline = "";
      let parte_position = Math.round(100 / (count_question + 1));
      let ball_collection;

      console.log(count_question);

      for (let i = 0; i < count_question; i++) {
        position = (i + 1) * parte_position;
        balls_on_the_timeline +=
          '<div class="btn-ball deactivated" style="left: calc(' +
          position +
          '% - 12px)"></div>';
      }
      ball_collection = $.parseHTML(balls_on_the_timeline);
      $(".timeline").append(ball_collection);

      $(ball_collection[0]).removeClass("deactivated");
      $(ball_collection[0]).addClass("active");
  ...
  $.each($(".quick-quiz-box"), function (index, element) {
    let questions = $(this).find(".quick-quiz-question");
    let options = $(this).find(".alternatives");
    let id_prefix = "radio_answer_";
    let id_counter = 0;
    let current_question = 0;
    let last_question = questions.length - 1;

    $(questions[last_question]).find("#btn-next").remove();
    $(questions[0]).show();

    $.each(options, function (i, el) {
      let new_id = id_prefix + id_counter;
      $(el).find(".ask").attr("id", new_id);
      $(el).find(".label-ask").attr("for", new_id);
      id_counter++;
    });

    let ball_collecion = buildTimeline(last_question, current_question);

    $.each(questions, function (i, el) {
      let bt_answer = $(el).find("#btn-answer");
      let bt_next = $(el).find("#btn-next");
      let feedback_box = $(el).find(".feedback");
      let feedbacks_radio_images = $(el).find(".answer");
      let radio_buttons = $(el).find(".ask");

      $(bt_answer).on("click", function () {
        if ($(el).find("input:checked")[0]) {
          $(feedback_box).show();
          $(bt_next).show();
          $(feedbacks_radio_images).show();
          $(radio_buttons).hide();
          $(bt_answer).hide();
        }
      });
      ...
      $(bt_next).on("click", function () {
        $(questions[current_question]).hide();
        console.log(ball_collecion[current_question]);
        $(ball_collecion[current_question]).removeClass("active");
        current_question++;
        $(questions[current_question]).show();

        let bt_answer = $(questions[current_question]).find("#btn-answer");
        let bt_next = $(questions[current_question]).find("#btn-next");
        let feedback_box = $(questions[current_question]).find(".feedback");
        let feedbacks_radio_images = $(questions[current_question]).find(
          ".answer"
        );
        let radio_buttons = $(questions[current_question]).find(".ask");

        $(radio_buttons).prop("checked", false);
        $(feedback_box).hide();
        $(bt_next).hide();
        $(feedbacks_radio_images).hide();
        $(radio_buttons).show();
        $(bt_answer).show();
        $(questions[current_question]).show();
        $(ball_collecion[current_question]).removeClass("deactivated");
        $(ball_collecion[current_question]).addClass("active");
      });
    });

    $(ball_collecion).on("click", function () {
      let _this = $(this);
      if (!_this.hasClass("deactivated") && !_this.hasClass("active")) {
        let reset_balls_to_index = ball_collecion.indexOf(this);

        for (let i = reset_balls_to_index + 1; i <= last_question; i++) {
          $(ball_collecion[i]).removeClass("active");
          $(ball_collecion[i]).addClass("deactivated");
        }

        $(ball_collecion[reset_balls_to_index]).removeClass("deactivated");
        $(ball_collecion[reset_balls_to_index]).addClass("active");

        $(questions[current_question]).hide();
        $(ball_collecion[current_question]).removeClass("active");

        current_question = reset_balls_to_index;
  ...
  // QUICK QUIZ BOX LOADER END

  // COL QUIZ
  $.each($(".quick-col-quiz-box"), function (index, element) {
    let columns = $(this).find(".columns-box");
    let columns_html = columns.find(".column-html");
    let columns_ol = $.parseHTML('<ol effect="fadein-left"></ol>');

    let answers = $(this).find(".answers-box");
    let answers_html = answers.find(".answer-html");
    let answers_ol = $.parseHTML('<ol effect="fadein-right"></ol>');
    let random_answers_html = shuffle([...answers_html]);

    let answer_button = $(this).find("#btn-answer");
    ...
    $(columns).append(columns_ol);
    $(answers).append(answers_ol);

    $.each(columns_html, function (index, element) {
      let column_li =
        '\
          <li class="column-li">\
            <span class="option-col">(' +
      ...
      column_li = $.parseHTML(column_li);
      $(columns_ol).append(column_li);

      $(column_li).on("click", function () {
        let self_parent = $(this);
        if (!$(self_parent).hasClass("inactive")) {
          try {
            $(selected_value).removeClass("active");
          } catch {}

          selected_value = self_parent;
          $(selected_value).addClass("active");
        }
      });
    });

    $.each(random_answers_html, function (index, element) {
      let real_index = element["real_index"];
      let element_value = element["value"];
      let answers_li =
        '\
      <li class="answers-li">\
        <span class="feedback-answer">(' +
        alphabet[real_index].toUpperCase() +
        ')</span>\
        <span class="gap" picked-col=""></span>\
        <span class="text">' +
        $(element_value)
          .text()
          .replace(/\u00a0/g, " ") +
      ...
      let gap;
      answers_li = $.parseHTML(answers_li);
      $(answers_ol).append(answers_li);

      gap = $(answers_li).find(".gap");
      $(gap).html("(&nbsp;&nbsp;)");
      $(answers_li).on("click", function () {
        let picked_col = $(gap).attr("picked-col");

        if (picked_col) {
          let id = $(gap).attr("picked-col");
          $(gap).html("(&nbsp;&nbsp;)");
          $(gap).attr("picked-col", "");
        ...
        if (selected_value !== null) {
          let id = $(selected_value).find(".option-col").attr("id");
          let option = $(selected_value).find(".option-col").text();
          $(gap).text(option);
          $(gap).attr("picked-col", id);
          $(selected_value).removeClass("active");
          $(selected_value).addClass("inactive");
          selected_value = null;
        }
      });
    });
    ...
    $(answer_button).click(function () {
      $(element).find(".feedback-answer").show();
    });
  });
  ...
  $("#preloader-container").css("z-index", 1000);
};
